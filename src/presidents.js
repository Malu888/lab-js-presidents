...
function getNames(presidentsArr) {
  const presidents = presidentsArr.map(function (presidentsArr) {
...
function getDemocraticPresidents(presidentsArr) {
  const presidents = presidentsArr.filter(presidentsArrParty)
  function presidentsArrParty(presidentsArr) {
...
function countYearsInOffice(presidentsArr) {
  return presidentsArr.reduce(function (totalYears, president) {
    const startWork = president.tookOffice
    const endWork = president.leftOffice
    if (endWork === null) {
      return totalYears
    }
    return totalYears + (endWork - startWork)
  }, 0)
}
